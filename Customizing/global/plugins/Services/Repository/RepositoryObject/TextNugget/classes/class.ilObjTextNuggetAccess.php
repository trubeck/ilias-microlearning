 ...
 * @version $Id$
 */
class ilObjTextNuggetAccess extends ilObjectPluginAccess implements ilConditionHandling
{
	...
	function _checkAccess($a_cmd, $a_permission, $a_ref_id, $a_obj_id, $a_user_id = 0)
	{
		...
		{
			case "read":
				if (!ilObjTextNuggetAccess::checkOnline($a_obj_id) &&
	...
	static function checkOnline($a_id)
	{
		...
		$set = $ilDB->query("SELECT is_online FROM rep_robj_xtxt_data ".
	...
	public static function checkCondition($a_trigger_obj_id, $a_operator, $a_value, $a_usr_id) {
		...
		$object = new ilObjTextNugget($ref_id);
		switch ($a_operator) {
