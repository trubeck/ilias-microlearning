 ...
 * @version $Id$
 */
class ilObjPalunoObjectAccess extends ilObjectPluginAccess implements ilConditionHandling
{
	...
	function _checkAccess($a_cmd, $a_permission, $a_ref_id, $a_obj_id, $a_user_id = 0)
	{
		...
		{
			case "read":
				if (!ilObjPalunoObjectAccess::checkOnline($a_obj_id) &&
	...
	static function checkOnline($a_id)
	{
		...
		$set = $ilDB->query("SELECT is_online FROM rep_robj_xpal_data ".
	...
	public static function checkCondition($a_trigger_obj_id, $a_operator, $a_value, $a_usr_id) {
		...
		$object = new ilObjPalunoObject($ref_id);
		switch ($a_operator) {
